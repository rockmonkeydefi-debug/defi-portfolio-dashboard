"""Ethereum chain connector implementation."""
...
class EthereumConnector(ChainConnector):
    """Ethereum mainnet RPC connector."""

    def __init__(self, rpc_url: Optional[str] = None):
        """Initialize Ethereum connector.
        
        Args:
            rpc_url: Optional RPC URL. If not provided, reads from ETHEREUM_RPC_URL env var.
        """
    ...
    def get_token_transfers(self, wallet: str) -> list[TokenTransfer]:
        """Fetch ERC-20 tokens for a wallet using Alchemy's native API.
        
        Uses alchemy_getTokenBalances which returns tokens with balances in one call.
        This is much more efficient than scanning logs.
        ...
        try:
            ...
            logger.info(f"Found {len(transfers)} unique tokens for {wallet} on Ethereum")
            return transfers
